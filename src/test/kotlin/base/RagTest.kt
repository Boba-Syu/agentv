...
class RagTest {
    ...
    fun ollamaTextEmbedder(testContext: VertxTestContext) {
        val ollamaTextEmbedder = OllamaTextEmbedderAbstract(embeddingEntity)
    ...
    fun ollamaVectorStoreAdapterTest(testContext: VertxTestContext) {
        ...
        val textSegmentVal = TextSegmentVal(
            text = """10.4 异步模式的工作流，发布到飞书、豆包等渠道后，工作流怎么运行
飞书、豆包目前不支持异步模式工作流。这类工作流在飞书、豆包运行时，会自动关闭异步模式，以维持运行。""".trimIndent(),
            metadata = mapOf("field" to "coze")
        )
        val ollamaTextEmbedder = OllamaTextEmbedderAbstract(embeddingEntity)
        val vector = ollamaTextEmbedder.embedText(textSegmentVal.text)

        val pgVectorStoreAdapter = PgVectorStoreAdapter(embeddingEntity)
        pgVectorStoreAdapter.initStorage()
        pgVectorStoreAdapter.storeVectors(listOf(vector), listOf(textSegmentVal))
        testContext.completeNow()
    }
    ...
    fun langChainAnswerGeneratorTest(testContext: VertxTestContext) {
        ...
        var answerQuestion = ragRepository.answerQuestion(
            "CITY THE ANIMATION 是什么时候播出的, 导演是谁",
            embeddingEntity,
            chatModelEntity
        )
        ...
        println("----------------")
        answerQuestion =
            ragRepository.answerQuestion("小城日常 是什么时候播出的, 导演是谁", embeddingEntity, chatModelEntity)
        println(answerQuestion.response)
        println(answerQuestion.supportSegments)
        testContext.completeNow()
    }
    ...
    @Test
    fun langChainAnswerGeneratorTest3(testContext: VertxTestContext) {
        val chatModelEntity = agentArmoryFactory.chatModelEntity("qwen3:8b", ModelSourceType.OLLAMA)
        val filter = listOf(
            MetadataFilter("field", "coze", FilterOperator.EQUAL)
        )
        val answerQuestion = ragRepository.answerQuestion(
            "为什么我的知识库导入不进去",
            embeddingEntity,
            chatModelEntity,
            filter
        )
        println(answerQuestion)
        testContext.completeNow()
    }

    @Test
    fun ollamaScoringModelTest(testContext: VertxTestContext) {
        val volcengineScoringModel = VolcengineScoringModel()
        val score = volcengineScoringModel.score(
